...
public class SpringDataConversationRepository implements ConversationRepository {
  ...
  public Conversation save(ConversationToCreate conversationToCreate, List<User> members) {
    ConversationEntity conversationEntity = ConversationEntity.from(conversationToCreate);
    conversationEntity.setUsers(UserEntity.from(members));
    ConversationEntity conversationEntitySaved = jpaConversationRepository.saveAndFlush(conversationEntity);
    return ConversationEntity.toDomain(conversationEntitySaved);
  }
  ...
  public int deleteByPublicId(UserPublicId userPublicId, ConversationPublicId conversationPublicId) {
    return
        jpaConversationRepository.deleteByUsersPublicIdAndPublicId(userPublicId.value(), conversationPublicId.value());
  }

  @Override
  public Optional<Conversation> getConversationByUserPublicIdAndPublicId(UserPublicId userPublicId, ConversationPublicId conversationPublicId) {
    return
        jpaConversationRepository.findOneByUsersPublicIdAndPublicId(userPublicId.value(), conversationPublicId.value())
            .map(ConversationEntity::toDomain);
  }
